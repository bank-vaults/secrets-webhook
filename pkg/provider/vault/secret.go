...
func mutateDockerCreds(ctx context.Context, secret *corev1.Secret, dc *common.DockerCredentials, injector *vaultinjector.SecretInjector) error {
	...
	for key, creds := range dc.Auths {
		...
		if isValidPrefix(string(authBytes)) {
			...
			dcCreds, err := injector.GetDataFromVaultWithContext(ctx, credentialData)
			if err != nil {
...
func mutateSecretData(ctx context.Context, secret *corev1.Secret, injector *vaultinjector.SecretInjector) error {
	data := make(map[string]string, len(secret.Data))
	for k := range secret.Data {
		data[k] = string(secret.Data[k])
	}

	convertedData, err := injector.GetDataFromVaultWithContext(ctx, data)
	if err != nil {
		return err
	}
...
// It supports three formats: "usr:pass", JSON keys, and valid vault paths.
